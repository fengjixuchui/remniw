...
antlrcpp::Any ASTBuilder::visitAllocExpr(RemniwParser::AllocExprContext *Ctx)
{
    exprIsLValue = false;
    visit(Ctx->expr());
    visitedExpr = std::make_unique<AllocExprAST>(SourceLocation{
        Ctx->getStart()->getLine(), Ctx->getStart()->getCharPositionInLine()}, std::move(visitedExpr));
    return nullptr;
}
...
antlrcpp::Any ASTBuilder::visitBasicAssignmentStmt(
    RemniwParser::BasicAssignmentStmtContext *Ctx)
{
    exprIsLValue = true;
    visit(Ctx->id());
    std::unique_ptr<ExprAST> LHS = std::move(visitedExpr);
...
antlrcpp::Any ASTBuilder::visitDerefAssignmentStmt(
    RemniwParser::DerefAssignmentStmtContext *Ctx)
{
    exprIsLValue = true;
    visit(Ctx->expr(0));
    std::unique_ptr<ExprAST> LHS = std::move(visitedExpr);
