target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"

@fmtstr = private unnamed_addr constant [5 x i8] c"%lli\00", align 1
@fmtstr.1 = private unnamed_addr constant [6 x i8] c"%lli\0A\00", align 1

define i64 @main() {
entry:
  %input3 = alloca i64, align 8
  %input1 = alloca i64, align 8
  %input = alloca i64, align 8
  %x = alloca i64, align 8
  %y = alloca i64, align 8
  %z = alloca i64, align 8
  %scanf = call i32 (i8*, ...) @scanf(i8* getelementptr inbounds ([5 x i8], [5 x i8]* @fmtstr, i32 0, i32 0), i64* %input)
  %0 = load i64, i64* %input, align 8
  store i64 %0, i64* %x, align 8
  %scanf2 = call i32 (i8*, ...) @scanf(i8* getelementptr inbounds ([5 x i8], [5 x i8]* @fmtstr, i32 0, i32 0), i64* %input1)
  %1 = load i64, i64* %input1, align 8
  store i64 %1, i64* %y, align 8
  %scanf4 = call i32 (i8*, ...) @scanf(i8* getelementptr inbounds ([5 x i8], [5 x i8]* @fmtstr, i32 0, i32 0), i64* %input3)
  %2 = load i64, i64* %input3, align 8
  store i64 %2, i64* %z, align 8
  %x5 = load i64, i64* %x, align 8
  %icmp.eq = icmp eq i64 %x5, 0
  br i1 %icmp.eq, label %if.then, label %if.else12

if.then:                                          ; preds = %entry
  %y6 = load i64, i64* %y, align 8
  %icmp.eq7 = icmp eq i64 %y6, 0
  br i1 %icmp.eq7, label %if.then8, label %if.else

if.then8:                                         ; preds = %if.then
  %z9 = load i64, i64* %z, align 8
  %printf = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([6 x i8], [6 x i8]* @fmtstr.1, i32 0, i32 0), i64 %z9)
  br label %if.end

if.else:                                          ; preds = %if.then
  %x10 = load i64, i64* %x, align 8
  %printf11 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([6 x i8], [6 x i8]* @fmtstr.1, i32 0, i32 0), i64 %x10)
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then8
  br label %if.end13

if.else12:                                        ; preds = %entry
  br label %if.end13

if.end13:                                         ; preds = %if.else12, %if.end
  ret i64 0
}

declare i32 @scanf(i8*, ...)

declare i32 @printf(i8*, ...)
