...
static void emitfuncs(void)
{
  ...
  print(
	"struct %Pstate *%Pimmed(struct %Pstate *s,int n)\n"
	"{\n"
...
static char *computekids(Tree t, char *v, char *bp, int *ip) {
	...
	if (p->kind == NONTERM) {
	  ...
	  for(i=0;i<t->nkids;i++)
	    bp=computekids(t->kids[i],
			   stringf("%s_child(%s,%d)", prefix, v,i), bp, ip);
	}
	return bp;
}
...
static void emitlabel(Nonterm start) {
	print(
"struct %Pstate *%Plabel(NODEPTR p) {\n%1%Plabel1(p);\n"
...
/*    print("%s%1delete s->cost[%P%S_NT].mset.getset();\n", pre, r->lhs);*/
  /* emit cost code */
  if (Tflag)
    print("%Ptrace(%Pnp, %d, c); ",
	  r->ern, r->lhs);
  print("%s%1s->cost[%P%S_NT] = c ;\n%s%1s->rule.%P%S = %d;\n",
	pre, r->lhs,  pre, r->lhs, r->packed);
  if (r->lhs->chain)
...
static void emitrule(Nonterm nts) {
	...
	print(
	      "int %Prule(struct %Pstate *state, int goalnt) {\n"
...
static void emit_exec_code()
{
	...
	print("\n\n#include <stdarg.h>\n\n");
        ...
        print(
            "void burm_exec(struct %Pstate *state, int nterm, ...) \n{"
            "\n%1va_list(ap);\n"
            "%1va_start(ap,nterm);\n\n"
	...
	print("%2default:\n%3PANIC(\"Bad nonterminal %%d in $exec\\n\", nterm);\n%3break;\n%2}\n%1else\n%2PANIC(\"Bad state for $exec in nonterminal %%d \\n\",nterm);\n%1va_end(ap);\n}\n\n");
...
static void emitstate(Term terms, Nonterm start, int ntnumber)
{
  ...
  print(
	"struct %Pstate *%Palloc_state(NODEPTR u,int op,int arity)\n{\n"
	"%1struct %Pstate *p, **k;\n"
#ifndef SYLIAO
  ...
  print(
	"struct %Pstate *%Plabel1(NODEPTR u) {\n"
	"%1int op, arity, i, immed_matched=0;\n"
        "%1COST c=COST_ZERO;\n"
	"%1struct %Pstate *s,**k;\n"
	"%1NODEPTR *children;\n"
	/*"%1%Passert(sizeof (int) >= sizeof (void *),PANIC(\"implementation failure\"));\n"*/
	"%1op=OP_LABEL(u);\n"
	"%1arity=%Parity[op];\n"
	"%1switch(op){\n"
	);
...
static void emit_cost_code(Rule rules, int nrules)
{
  ...
  print(
	"int %Pcost_code(COST *_c, int _ern,struct %Pstate *_s)\n{\n"
	"%1NODEPTR *_children;\n"
        "%1struct %Paction *act;\n"
	"%1switch(_ern){\n"
	"%1default:\n"
