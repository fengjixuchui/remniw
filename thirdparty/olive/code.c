...
#define SKIP_UNTIL(s,delim) while(*s && strchr(delim,*s)==0) INC(s)
...
static char *Code_print_block(char *s,Code *c,
			      Rule r,char *prefix,int null_allowed)
{
  line_within_code = 0;
  while(*s)
    switch(*s){
    ...
    case '\'':
    case '"':
      s=Code_print_quoted(s);
      break;
    case '$':
      s=Code_print_dollar(s,c,r,prefix);
      break;
    case '}':
      return s;
    default:
...
Type* Type_get_return(Code* rty)
{
  ...
  for (; i < Code_ub(rty); i++) {
    ...
    case ' ':
      break;
    default:
      X_arrayc_extend(buf,Code_fetch(rty,i));
      break;
    }
  }
  X_arrayc_extend(buf,'\0');
